...
- `consumer/sc_paths.php` loads the first `paths.php` it finds in `__DIR__/lib`, `../lib`, `../../lib` or `/web/lib`. If there is none, it defines the same five path functions with the `/web` default. Edit it here and copy it out again. Never edit a copy.
  Copies: signCollect-v2, zin, hh, studio_beta, viconDashboard, sC-Animation-PP, blendAnims, mocap, mocapStudio (root and `lab/`) and the stack's `interface_deploy/web_extra/`. All match this one.
...
- `consumer/sc_paths.py` is the Python twin: `sc_root()`, `sc_path()`, `sc_dir()`, the same 4 names, and `sc_setting(key, default)`. Root: `SC_WEB_ROOT` env var, then `SC_WEB_ROOT=` in `$SC_ENV_FILE` or `/web/.env`, then `/web`. Copies: mocap, pythonCron, viconSync, zin, mocapDataPackage.
- The PHP resolver is also copied into s3b_server, s3b_glb, s3b_viewer, client_monitor_dashboard and mocapDataPackage, which the stack does not deploy.
